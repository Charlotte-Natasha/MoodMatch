...
import { isLoggedIn as isSpotifyConnected, loginWithSpotify } from "../services/SpotifyAuth";
import { useNavigate } from "react-router-dom";

const WelcomeScreen = ({ redirecting = false }) => {
  ...
  // Check login status
  useEffect(() => {
    const userIsLoggedIn = isSpotifyConnected();
    setLoggedInState(userIsLoggedIn);

    if (userIsLoggedIn && !redirecting) {
      setAutoRedirect(true);
      const timer = setTimeout(() => {
        navigate("/moods-select", { replace: true });
      }, 1500); // short delay for "Welcome Back" animation
  ...
  // Loading state
  if (loggedInState === null || redirecting) {
    return (
      <div
      ...
      >
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/60 text-sm">
            {redirecting ? "Connecting to Spotify..." : "Loading..."}
          </p>
        </div>
      </div>
    );
      ...
      <div className="relative z-10 flex flex-col items-center justify-center max-w-4xl w-full">
        ...
        <div className="w-full max-w-md text-center px-4">
          ...
          <button
            ...
            disabled={autoRedirect || redirecting}
